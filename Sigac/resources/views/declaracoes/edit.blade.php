            ...
            <form action="{{ route('declaracoes.update', $declaracao->id) }}" method="POST">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('tipo') is-invalid @enderror" 
                                ...
                                <option value="matricula" {{ old('tipo', $declaracao->tipo) == 'matricula' ? 'selected' : '' }}>Matrícula</option>
                                <option value="frequencia" {{ old('tipo', $declaracao->tipo) == 'frequencia' ? 'selected' : '' }}>Frequência</option>
                                <option value="conclusao" {{ old('tipo', $declaracao->tipo) == 'conclusao' ? 'selected' : '' }}>Conclusão</option>
                                <option value="outra" {{ old('tipo', $declaracao->tipo) == 'outra' ? 'selected' : '' }}>Outra</option>
                            </select>
                            @error('tipo')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control @error('data_emissao') is-invalid @enderror" 
                                   id="data_emissao" name="data_emissao" value="{{ old('data_emissao', $declaracao->data_emissao) }}">
                            @error('data_emissao')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control @error('observacoes') is-invalid @enderror" 
                              id="observacoes" name="observacoes" rows="3">{{ old('observacoes', $declaracao->observacoes) }}</textarea>
                    @error('observacoes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
